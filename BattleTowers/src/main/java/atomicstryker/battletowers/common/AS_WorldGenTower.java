...
public class AS_WorldGenTower {
	...
	public int getChosenTowerOrdinal(World world, Random random, int ix, int jy, int kz) {
		...
		// FMLLog.getLogger().info("Battle Tower Snow: "+countSnow+" Sand: "+countSand+" Water: "+countWater+" else: "+countElse);
		
		int[] nums = { countWater, countSnow, countSand, countFoliage, countElse };
		Arrays.sort(nums);
		int result = nums[nums.length - 1];
		
		// FMLLog.getLogger().info("Battle Tower Picked max value of "+result);
		...
		// FMLLog.getLogger().info("Battle Tower selected: "+towerChosen);
	...
	public void generate(World world, int ix, int jy, int kz, int towerchoice, boolean underground) {
		...
		// FMLLog.getLogger().info("Battle Tower Generate(" + ix + " " + jy + " " + kz + " type "+towerChosen+" isUnderground " + underground +")");
		...
		// FMLLog.getLogger().info("Battle Tower Generate wall of " + towerWallBlockID.getUnlocalizedName());
		...
		for (; builderHeight < maximumHeight; builderHeight += 7) // builderHeight jumps floors
		{
			...
			if ((!underground && topFloor) || (underground && floor == 1)) {
				...
				if (towerChosen != TowerTypes.Null) {
					...
					TileEntityMobSpawner tileentitymobspawner = (TileEntityMobSpawner) world.getTileEntity(ix + 2, builderHeight + 6, kz + 2);
					if (tileentitymobspawner != null) {
						tileentitymobspawner.func_145881_a().setEntityName(getMobType(world.rand));
					}
					
					world.setBlock(ix - 3, builderHeight + 6, kz + 2, Blocks.mob_spawner, 0, 3);
					tileentitymobspawner = (TileEntityMobSpawner) world.getTileEntity(ix - 3, builderHeight + 6, kz + 2);
					if (tileentitymobspawner != null) {
						tileentitymobspawner.func_145881_a().setEntityName(getMobType(world.rand));
					}
				} else {
			...
			if (towerChosen != TowerTypes.Null) {
				...
				for (int chestlength = 0; chestlength < 2; chestlength++) {
					...
					TileEntityChest tileentitychest = (TileEntityChest) world.getTileEntity(ix - chestlength, builderHeight + 7, kz + 3);
			...
			if (towerChosen != TowerTypes.Null) {
				for (int l3 = 0; l3 < (floor * 4 + towerChosen.ordinal()) - 8 && !topFloor; l3++) // random hole poker
				{
					...
					k4 += ix;
					j6 += kz;
					if (world.getBlock(k4, k5, j6) == towerFloorBlockID && world.getBlock(k4, k5 + 1, j6) != Blocks.mob_spawner) {
		...
		FMLLog.getLogger().info("Battle Tower type " + towerChosen + " spawned at [ " + ix + " | " + kz + " ], underground: " + underground);
	}
	...
	private boolean isBannedBlockID(Block iD) {
		return (iD == Blocks.yellow_flower || iD == Blocks.red_flower || iD == Blocks.brown_mushroom_block || iD == Blocks.red_mushroom_block || iD == Blocks.cactus || iD == Blocks.pumpkin || iD == Blocks.lava);
	}
